%rebiuld 3Dpoints cloud with 'cl.csv' and 'cr.csv'
...
cl=0;
   ...
   count=1;
    ...
    cl=csvread('cl.csv'); %读入左右图片点坐标   
    cr=csvread('cr.csv');

for i=1: POI_point_count %左图opencv去畸变
				 y11 = (cl(i,2) - cyl) / fyl;
				 x11 = (cl(i,1) - cxl - fsl*y11) / fxl;
				 r2 = x11*x11 + y11*y11;
    ...
    for k=1:50 %迭代次数
	      for i=1: POI_point_count
				 ...
				 r2 = x11*x11 + y11*y11;
                 x1 = ((cl(i,1) - cxl - fsl*y11) / fxl - 2 * p1l*x11*y11 - p2l*(r2 + 2 * x11*x11))/ (1 + k1l*r2 + k2l*r2*r2 + k3l*r2*r2*r2);
				 y1 = ((cl(i,2) - cyl) / fyl - 2 * p2l*x11*y11 - p1l*(r2 + 2 * y11*y11))/ (1 + k1l*r2 + k2l*r2*r2 + k3l*r2*r2*r2);
...
for k=1:50 %迭代次数
	      for i=1: POI_point_count
				 ...
				 r2 = x11*x11 + y11*y11;
				 x1 = ((cr(i,1) - cxr - fsr*y11) / fxr - 2 * p1r*x11*y11 - p2r*(r2 + 2 * x11*x11))/ (1 + k1r*r2 + k2r*r2*r2 + k3r*r2*r2*r2);
				 y1 = ((cr(i,2) - cyr) / fyr - 2 * p2l*x11*y11 - p1r*(r2 + 2 * y11*y11))/ (1 + k1r*r2 + k2r*r2*r2 + k3r*r2*r2*r2);
				centersnew1_1(i,1) = cxr + fxr*x1+ fsr*y1;
				centersnew1_1(i,2) = cyr + fyr*y1;   
          end  
 end
...
csvwrite(strcat('pc',num2str(count),'.csv'), centers1w);
axis equal
hold on
